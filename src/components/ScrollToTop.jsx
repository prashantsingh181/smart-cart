...
const ScrollToTop = ({ scrollRef }) => {
  ...
  return (
    <button
      className={`absolute bottom-8 right-3 md:right-5 bg-theme-color text-button-text-color rounded-full h-12 aspect-square flex justify-center items-center ${isVisible ? "flex" : "hidden"
        }`}
      onClick={scrollToTop}
    >
      <FaArrowUp />
